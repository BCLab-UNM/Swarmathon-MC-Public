...
#define STATE_MACHINE_HEADING 7
        ...
        case STATE_MACHINE_PICKUP: {
            ...
            if (targetDetected && !targetCollected) {
                ...
                if (result.pickedUp) {
                    ...
                    stopCount = true;//stop counting the cubes to that value stays at 0;
                    //tell the server that we picked up a target and saw this many of them in the process
...
void targetHandler(const apriltags_ros::AprilTagDetectionArray::ConstPtr& message) {
    ...
    if (message->detections.size() > 0 && !reachedCollectionPoint) {
        ...
        if((countCubes > targetCounter) && (!stopCount)){
            //ROS_INFO("See this many targets: %d", targetCounter);
            targetCounter = countCubes;
        }
